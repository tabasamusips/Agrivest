...
class PaymentsService {
    ...
    ledger;
    daraja;
    constructor(db, ledger, daraja) {
        this.db = db;
        this.ledger = ledger;
        this.daraja = daraja;
    }
    /** Investor taps "Deposit" -> STK push to their phone. Money lands on callback. */
    async initiateDeposit(user, amountCents, phone) {
        const amountKES = (0, daraja_js_1.centsToWholeKES)(amountCents);
        const res = await this.daraja.stkPush({
            amountKES, phone, accountReference: `AV-${user}`, description: "Agri Vest deposit",
        });
        if (res.ResponseCode !== "0")
            throw new errors_js_1.LedgerError(`STK push rejected: ${res.CustomerMessage}`);
        await this.db.query(`INSERT INTO payment_intent(id, kind, account_ref, amount, phone) VALUES ($1,'deposit',$2,$3,$4)`, [res.CheckoutRequestID, user, amountCents, phone]);
        return { checkoutRequestId: res.CheckoutRequestID };
    }
    /** Daraja POSTs here when the customer approves (or cancels) the STK prompt. */
    async handleStkCallback(result) {
        return (0, db_js_1.withTx)(this.db, async () => {
            const rows = (await this.db.query("SELECT * FROM payment_intent WHERE id = $1 FOR UPDATE", [result.checkoutRequestId])).rows;
            if (!rows.length)
                throw new errors_js_1.LedgerError(`unknown CheckoutRequestID ${result.checkoutRequestId}`);
            const intent = rows[0];
            ...
            await this.db.query("UPDATE payment_intent SET status='completed', receipt=$2 WHERE id=$1", [result.checkoutRequestId, result.receipt]);
        });
    }
    /**
     ...
     */
    async initiateWithdrawal(user, amountCents, phone) {
        const amountKES = (0, daraja_js_1.centsToWholeKES)(amountCents);
        ...
        await this.db.query(`INSERT INTO payment_intent(id, kind, account_ref, amount, phone, status)
       VALUES ($1,'withdrawal',$2,$3,$4,'pending')`, [res.OriginatorConversationID, user, amountCents, phone]);
        return { conversationId: res.OriginatorConversationID };
    }
    async handleB2CResult(result) {
        return (0, db_js_1.withTx)(this.db, async () => {
            const rows = (await this.db.query("SELECT * FROM payment_intent WHERE id = $1 FOR UPDATE", [result.originatorConversationId])).rows;
            if (!rows.length)
                return;
            const intent = rows[0];
            if (intent.status !== "pending")
                return;
            if (result.success) {
                await this.db.query("UPDATE payment_intent SET status='completed', receipt=$2 WHERE id=$1", [result.originatorConversationId, result.receipt ?? null]);
            }
            else {
